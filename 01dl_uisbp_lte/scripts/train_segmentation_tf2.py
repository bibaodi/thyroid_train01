...
if __name__ == '__main__':
    ...
    loss = CrossEntropyJaccardLoss(jaccard_weight=1.0,
                                   ...
                                   special_indexs=[list(labels).index(args.keylabel), list(labels).index('IM')],
                                   special_weights=[10, 10])
    ...
    learn.fit(args.learning_rate, n_cycle=9, cycle_len=2, metrics=metrics, monitor=(monitor, 'max'))

    # fine tuning default n_cycle=4, cycle_len=1, cycle_mult=3
    learn.fit(args.learning_rate/5, n_cycle=2, cycle_len=1, cycle_mult=3, metrics=metrics, monitor=(monitor, 'max'))
