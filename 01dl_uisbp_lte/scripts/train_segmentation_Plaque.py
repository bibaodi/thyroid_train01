...
if __name__ == '__main__':
    ...
    parser.add_argument('--datapath', dest='datapath', type=str, required=True,
                        help='Path to folder where numpy data is stored.')
    parser.add_argument('--dataset', dest='dataset', type=str, default='apr',
                        help='Dataset name. Valid options are [apr, may, multi, binary]')
    parser.add_argument('--model', dest='model', type=str, default='linknet18',
                        help='Model architecture to use [unet, linknet18, linknet34]')
    parser.add_argument('--start_channel_depth', dest='start_channel_depth', type=int,
                        default=32, help='Starting channel depth for encoder [default 32]')
    parser.add_argument('--dropout', dest='dropout', type=float, default=0.5,
                        help='Dropout in encoder [default 0.5]')
    parser.add_argument('--size', dest='size', type=int, default=96,
                        help='Square image size to train on.')
    parser.add_argument('--outdir', dest='outdir', type=str, default='./results/',
                        help='Output directory for saved models and any other outputs.')
    parser.add_argument('--seed', dest='seed', type=int, default=898,
                        help='random number seed for train/validation split.')
    parser.add_argument('--batch_size', dest='batch_size', type=int, default=32,
                        help='Training Batch Size.')
    parser.add_argument('--learning_rate', dest='learning_rate', type=float, default=5e-3,
    ...
    parser.add_argument('--use_binary_output', dest='use_binary_output', action='store_true',
                        help='Use binary output for BP even though model was trained with multi-class input')
    parser.add_argument('--flip_data', dest='flip_data', action='store_true',
                        help='Option to orient all images so that they are right-oriented during training')
    parser.add_argument('--channels_first_output', dest='channels_first_output', action='store_true',
                        help='Option to output model in channels-first format')
    parser.add_argument('--combine_train_valid', dest='combine_train_valid', action='store_true',
                        help='Option to combine the training and validation data to create a larger training set.')
    parser.add_argument('--gpuid', dest='gpuid', type=int,
                        default=0, help='Which GPU device to use [default 0]')
    parser.add_argument('--keylabel', dest='keylabel', type=str, required=True,
                        help='Key class label of segmentation.')
    parser.add_argument('--use_augmentation', dest='use_augmentation', action='store_true',
    ...
    if args.use_augmentation:
        augs = tuple([
            ...
            Augmentation(fn=random_zoom, kwargs={'zoom_range': (0.7, 1.3), 'fill_mode': 'nearest'}),
            Augmentation(fn=random_rotation, kwargs={'rg': 15.0, 'fill_mode': 'nearest'}),
            Augmentation(fn=random_shear, kwargs={'intensity': 0.5, 'fill_mode': 'nearest'}),
            Augmentation(fn=random_shift, kwargs={'wrg': 0.2, 'hrg': 0.2, 'fill_mode': 'nearest'}),
    ...
    if args.flip_data:
        ...
        ds = DataSet((X_train_flip, Y_train_flip), (X_valid_flip, Y_valid_flip), batch_size=args.batch_size,
                     test=(X_test_flip, Y_test_flip), preprocess_fn=preprocess_fn, augs = augs)
    else:
        ds = DataSet((X_train, Y_train), (X_valid, Y_valid), batch_size=args.batch_size, test=(X_test, Y_test),
                    preprocess_fn=preprocess_fn, augs = augs)
    ...
    if args.model in ['linknet18', 'linknet34']:
        ...
        if args.model == 'linknet18':
            arch = linknet18((args.size, args.size, 1), start_ch=args.start_channel_depth,
                             dropout=args.dropout, out_ch=Y_train.shape[-1])
        else:
            arch = linknet34((args.size, args.size, 1), start_ch=args.start_channel_depth,
                             dropout=args.dropout, out_ch=Y_train.shape[-1])

        outdir = f'{args.outdir}/{args.model}_{args.start_channel_depth}_{args.dropout}_{args.dataset}_{args.size}'

    elif args.model == 'unet':
        dropout = [args.dropout] * 5 + [0] * 4
        arch = Unet((args.size, args.size, 1), start_ch=args.start_channel_depth, inc_rate=2,
                     dropouts=dropout, residual=True, maxpool=False, use_shortcut=True, out_ch=Y_train.shape[-1])
        outdir = f'{args.outdir}/{args.model}_{args.start_channel_depth}_{args.dropout}_{args.dataset}_{args.size}'
    ...
    # setup loss
    loss = CrossEntropyJaccardLoss(jaccard_weight=1.0, class_weights=weights, num_classes=Y_train.shape[-1],
                                   bp_index=list(labels).index(args.keylabel), bp_weight=2)
    ...
    learn.load(f'{outdir}/weights.h5', weights_only=True)
    model = learn.model
    ...
    np.save(f'{outdir}/ypred.npy', ypred)
    np.save(f'{outdir}/ytest.npy', Y_test)
    ...
    output_graph_name = f"output_graph_{output_extras}.pb" if output_extras else "output_graph.pb"
