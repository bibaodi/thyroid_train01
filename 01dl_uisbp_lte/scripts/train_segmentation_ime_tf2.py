...
if __name__ == '__main__':
    ...
    if args.size not in [96, 128, 160, 224, 416, 448]:
        raise ValueError('Input size must be in [96, 128, 160, 224, 416, 448] for segmentation')
    # get data
    ...
    # setup loss
    loss = CrossEntropyJaccardLoss(jaccard_weight=2.0,
                                   class_weights=weights,
                                   num_classes=Y_train.shape[-1],
                                   special_indexs=[list(labels).index(args.keylabel), list(labels).index('Plaque')],
                                   special_weights=[1.05, 1.1])
    ...
    # warmup
    learn.fit(1e-5, n_cycle=10, cycle_len=1, metrics=metrics, monitor=(monitor, 'max'))

    # Initial training default n_cycle=20, cycle_len=2
    learn.fit(args.learning_rate, n_cycle=10, cycle_len=2, metrics=metrics, monitor=(monitor, 'max'))
    ...
    model_file_h5 = learn.save(f'xinmy_{args.dataset}')
