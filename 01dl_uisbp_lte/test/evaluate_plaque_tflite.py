...
_logpath_ = r'evaluate_plaque_model.log'
_logger_ = None
...
LabelList_plaque = ['CA', 'JV', 'Plaque', 'Thyroid', '_background_']
...
def read_image_and_correct_it(imagefile=None, zoom=False, zoomed_size=448, clahe=True):
    ...
    #yolo model need rgb rather than gray --eton@250913
    img = cv2.imread(imagefile)
    if clahe and img.shape[2] == 1:
...
def generate_image_result(index, outputdir, img_info_touple_s, X, y_groundtrouth, Ypred,
    ...
    inputimg, framename, imagepath = prepareBaseImage(index, outputdir,
                                                      imgs_indx, X, img_outsize=img_outsize)
    ...
    drawFooterMessage(outimgeexp, "Predict-Frame=%s-Dice=%.2f" % (framename, dice), ratio)
...
def createresult(index, outputdir, img_info_touple_s, images_struct, y_groundtrouth, ypreds,
    ...
    _, contours, hierarchy = cv2.findContours(
            (ypreds[index][..., key_label_ch] > SEG_THRESHOLD).astype(np.uint8), cv2.RETR_TREE,
            cv2.CHAIN_APPROX_SIMPLE)
    ...
    metrics4 = calculate_4_matrics_by_dice(dice, (img_info_touple_s[index][1]), (ypred_sum, len(contours)),
                                   KEY_LABEL)
    ...
    if img_outsize >= 96:
        generate_image_result(index, outputdir, img_info_touple_s, cropped_imgs, y_groundtrouth, ypreds,
                            model_label_names, masks_label_names, labelcolors, modelPara, dice)
...
def use_tflite_predict(imgs, tf_interpreter):
    ...
    for img in imgs:
        ...
        if list(x.shape) == input_shape.tolist():
            ...
            raise ValueError(f"image shape error: model={input_shape}, img={x.shape}")
