...
History：
    20200202-write to detach uisbp 
    > no thresholds work
    20200213- support both xiaobaishi(output is not normalization) and my original model(output is normalization)
    20200306- suport threshold or softmax to apply model output.
    20200331- masks labels merged into models labels names. and the groundtruth picture use original image.
    20200331- 修改模型指标的计算从基于DICE计算TP-FN改为直接基于pixel计算TP-FN, 单张图片不体现TP与否，直接统计为所有图片的像素统计模型结果
    20200401- 支持一次计算同时输出CA-Plaque两个目标的统计数值

TODO:
    1. 同一张图片出现多个目标， 一个是FP一个是TP的时候，需要能够识别到
    2. 血管的小的连通区域裁剪加上后会更好一些
'''
...
_logpath_ = r'evaluate_CA.Plaque_model.log'
_logger_ = None
...
def calcDice(y,
    ...
    for target_label in target_labels:
        ...
        ch = model_label_names.index(target_label)
        ypred = y[..., ch] > thresh
        ch = masks_label_names.index(
            target_label) if target_label in masks_label_names else -1
        if ch < 0:
        ...
        dice_coef.append(key_dice[0])
    return dice_coef


def calculate_4_matrics_by_dice(dice, labels, inference, key_label):
    """
    ...
    if not isinstance(labels, set):
        print(f"labels should be set type:{labels}")
        return None
...
def calculate_4_matrics_by_pixels(inference:np.ndarray, ground_truth:np.ndarray, label_dict:dict, keylabel:str):
    ...
    #todo 一次计算所有想要的目标的结果
    results = {}
    kch = label_dict[keylabel] if keylabel in label_dict else 0
    for (k, v) in label_dict.items():
        if 'JV' in k or '_background_' in k:
            continue
        result = {"TP": 0, "TN": 0, "FP": 0, "FN": 0}
        ch = v
        ...
        results[k] = [result["TP"], result["TN"], result["FP"], result["FN"]]
    #(results, [result["TP"], result["TN"], result["FP"], result["FN"]])
    return results
...
def get_imgs_masks_labels(img_root_dir="", label_names: list=[], extension='png', size=448, keyLabel='', abandon_list=[], black_it_ratio=[]):
    ...
    return img_array, masks_nd_array, label_dict, np.array(imgs_index), np.stack(masks_1d, axis=0), np.stack(cropped_imgs, axis=0)
...
def generate_image_statistics_csv(filename, videos_data):
    """
        以每张图片的结果为一行输出到csv文件. 包括dice以及TP,FP等指标值
    """
    detailtable = []
    csv_head = ['video', 'image']
    csv_head.extend(['CA.dice', 'CA.TP', 'CA.TN', 'CA.FP', 'CA.FN'])
    csv_head.extend(['PL.dice', 'PL.TP', 'PL.TN', 'PL.FP', 'PL.FN'])
    for img_datas in videos_data:  # iterate erery video-folder
...
def generate_video_dice_statistics_csv(filename, videos_data):
    """
...
def generate_model_classification_matrics_csv(filename, videos_data):
    """
    ...
    metrics_list = []
    csv_head = ['video']
    raw_target_head = ['dice_mean', 'accuracy', 'precision', 'recall', 'F1', 'TP',
        'TN', 'FP', 'FN']
    target_head = {'CA':[], 'PL':[]}
    for t in target_head:
        for ele in raw_target_head:
            nele = f"{t}.{ele}"
            target_head[t].append(nele)
    csv_head.extend(target_head['CA'])
    csv_head.extend(target_head['PL'])
    framedetails_names = ('image', 'dice', 'TP', 'TN', 'FP', 'FN')
    total_tp = np.array([0, 0])
    total_tn = np.array([0, 0])
    total_fp = np.array([0, 0])
    total_fn = np.array([0, 0])
    for video_data in videos_data:  # iterate erery video-folder
        videoname = video_data['videoname']
        img_datas_array = np.array(
            video_data['framedetails']
        )  # shape=(img_count, 6(image, dice, TP~*4))
        row = [videoname]
        imgs_datas = img_datas_array[:, 1:].astype(float)
        metrics_group_length = 5
        if imgs_datas.shape[-1] > metrics_group_length:
            targets = int(imgs_datas.shape[-1] / metrics_group_length)
        for targ in range(targets):
            dice_array = imgs_datas[:, (0+metrics_group_length*targ)].astype(float)
            dice_array[dice_array == DICE_CONST] = DICE_TN
            dice_mean = np.mean(dice_array)
            row.append(dice_mean)

            tp, tn, fp, fn = (np.sum(imgs_datas[:, i+(1+metrics_group_length*targ)]) for i in range(4))
            total_tp[targ] += tp
            total_tn[targ] += tn
            total_fp[targ] += fp
            total_fn[targ] += fn
            ...
            row.extend([accuracy, precision, recall, f1, tp, tn, fp,
                fn])  # get all image-info in one video-folder
    ...
    for targ in range(targets):
        metrics_array = np.array(metrics_list)[:, (1+len(raw_target_head)*targ):].astype(np.float)
        total_row.extend([np.mean(metrics_array[:, 0]), total_acc[targ], total_precision[targ],\
            total_recall[targ], total_f1[targ], total_tp[targ], total_tn[targ], total_fp[targ], total_fn[targ]])
...
def generate_image_result(index, outputdir, imgs_idx, X, y_groundtrouth, Ypred,
    ...
    """
    :param dice list of float, len is >=2
    根据预测结果生成图片预测结果, 方面进行结果效果查看
    """
    if len(dice) < 2:
        getLogger().error(f"dice must have as least 2 values")
        raise 
    ...
    getLogger().debug(f"@@@debug: result image path:{imagepath}")
    ...
    drawFooterMessage(base,
                      f"Model: {model_name, model_size},frm{imgs_idx}", ratio)
    ...
    if OUTPUT_FULL_EXPECTION:
        ...
        drawFooterMessage(outimge,
                          "GT-"+datetime.datetime.now().strftime('%Y/%m/%d %H:%M'), ratio)
    ...
    drawFooterMessage(outimgeexp, f"Predict:ca={dice[0]:.2f},pl={dice[1]:.2f}", ratio)
...
def createresult(index, outputdir, imgs_idx, images_struct, y_groundtrouth, ypreds,
    ...
    dice_coefs= calcDice(ypreds[index], model_label_names, y_groundtrouth[index], masks_label_names,
                                               target_labels=['CA', 'Plaque'],
                                               thresh=SEG_THRESHOLD)

    metrics4 = calculate_4_matrics_by_pixels(ypreds[index], y_groundtrouth[index], label_dict,
                                   KEY_LABEL)

    full_image_name_index = imgs_idx[index]
    framename = full_image_name_index[-4:]
    one_image_result = ['frm-' + framename]  # frm-ca(dice, tp,...)-plaque(dice,tp,...) total 11 elements
    one_image_result.append(dice_coefs[0])
    one_image_result.extend(metrics4['CA'])
    one_image_result.append(dice_coefs[1])
    one_image_result.extend(metrics4['Plaque'])
    if img_outsize >= 96:
        generate_image_result(index, outputdir, imgs_idx, cropped_imgs, y_groundtrouth, ypreds,
                            model_label_names, masks_label_names, labelcolors, modelPara, dice_coefs)
...
def HandleOneFrameFolder(image_dir, out_dir, modelPara):
    ...
    model_size = modelPara['size']
    imgs, gt_nd, label_dict, imgs_idx, masks_1d, cropped_imgs = get_imgs_masks_labels(image_dir, modelPara['label_names'], 
        size=model_size, keyLabel=KEY_LABEL)
    masks_label_names = modelPara['label_names']
    getLogger().info(f"imgs.shape={imgs.shape}, nd.shape={gt_nd.shape}, 1d.shape={masks_1d.shape}, imgs_idx={imgs_idx.shape}")
    ...
    for i in range(gt_nd.shape[0]):
        res = createresult(i, finaloutputdir, imgs_idx, images_struct,
