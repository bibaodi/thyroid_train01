...
Usage:
    evaluate_linket_models.py (need to modify source code when test different models or testSets)
...
def createresult(index, outputdir, imgs_idx, X, Y_Original, Y, Ypred,
    ...
    """
    getLogger().debug(f'@@@debug X={X.shape}Y={Y_Original.shape}')
...
def use_tflite_predict(xs, tf_interpreter):
...
def generate_model_classification_matrics_csv(filename, images_datas):
    ...
    for img_datas in images_datas:  # iterate erery video-folder
        ...
        precision = (tp) / (tp + fp) if ((tp+fp)>0) else MINIMUM
        recall = (tp) / (tp + fn) if ((tp+fn)>0) else MINIMUM
        f1 = (2 * tp) / (2 * tp + fp + fn) if ((tp+fp+fn)>0) else MINIMUM
...
def mainProcess(img_root_dir, out_dir, modelPara):
    if os.path.isfile(modelPara['dset']):
        model_part = os.path.basename(modelPara['dset']).split('.tflite')[0]
    else:
        raise ValueError(f"modelPara['dset'] must be tflite file!!!")
    out_dir = os.path.join(out_dir,
                           model_part + '_' + str(modelPara['size']))
    imgdir_names = []
...
def main(confirmed):
    ...
    img_root = r'F:\workspace\testSet'
    global _logger_, _logpath_
    _logpath_ = os.path.join(img_root, 'precrop.log')
    img_subdirs = [
        #'testSet5.0_lit',
        'testSet_plaque_v1.0',
        #'testSet_plaque_v1.0-filterarea',
        #'ValidationSet5.0',
    ...
    threshs = [0.1, 0.26, 1e-4, 0.26]
    labels_thresh = {i: j for i, j in zip(labellist, threshs)}
    # models = [ ('plaquev3.0_0122', 448), ('plaquev3.0.aug_0122', 448), ('plaquev3.0_0125', 160)]

    models = [(r"F:\workspace\tf2.0_out\linknet18_32_0.5_multi_96\linknet_model_7.0posi.ca.tflite", 96)]
