...
_logpath_ = r'evaluate_plaque.log'
_logger_ = None
...
def get_one_image_dice_coef(ypred, ytrue, smooth=MINIMUM):
    print(f"@maoguanqiao: pred={np.sum(ypred)}, gt={np.sum(ytrue)}")
...
def correct_image_size(img_array, zoom=False, zoomed_size=0):
    ...
    print(f"@maoguanqiao: left-right{true_width}")
...
def get_imgs_masks_labels(img_root_dir="", extension='png', size=448, keyLabel='', abandon_list=[], black_it_ratio=[]):
    ...
    for fname in tqdm.tqdm(filenames, ncols=80, desc="Processing Files:"):
        ...
        if keyLabel:  # keep keylabel mask when it overlapps with others
            ...
            mask_1d = shapes_to_label(img.shape, data['shapes'],
                                        label_dict)
        masks_1d.append(mask_1d)
        mask_nd = transfer_mask_from_1d_to_nd(mask_1d, label_dict)
        masks_nd.append(mask_nd)
    print(f"debug: masks_nd[0].shape:{masks_nd[0].shape}\nmasks_1d[0].shape:{masks_1d[0].shape}")
    # get arrays
    img_list = [img[..., None] for img in imgs]
    img_array = np.stack(img_list, axis=0)
    #masks_nd_array = np.zeros((len(masks_nd), size, size, len(label_dict)),
    #                        dtype=np.uint8)
    #for ct, mask in enumerate(masks_nd):
    #    masks_nd_array[ct, ...] = np.dstack([
    #        mask[..., ii][..., None] for ii in range(mask.shape[-1])
    #    ])
    masks_nd_array = np.stack(masks_nd, axis=0)
    print(f"debug: masks_nd_array.shape:{masks_nd_array.shape}")
    return img_array, masks_nd_array, label_dict, np.array(imgs_index), np.stack(masks_1d, axis=0)
...
def createresult(index, outputdir, imgs_idx, X, y_groundtrouth, ypreds,
    ...
    if modelPara['use_threshold'] == 0:
        ...
        getLogger().debug(f'createresult: ypreds[index].shape{ypreds[index].shape}')
        ypreds[index] = np.argmax(ypreds[index], axis=-1)
        getLogger().debug(f'createresult: ypreds[index].shape{ypreds[index].shape}')
        ypreds[index] = transfer_mask_from_1d_to_nd(ypreds[index], label_dict)
    dice, top, bottom, ypred_sum, _ = calcDice(ypreds[index],
                                               model_label_names,
    ...
    if img_outsize >= 96:
        generate_image_result(index, outputdir, imgs_idx, X, y_groundtrouth, ypreds,
                            model_label_names, masks_label_names, labelcolors, modelPara, dice)
...
def HandleOneFrameFolder(image_dir, out_dir, modelPara):
    ...
    model_size = modelPara['size']
    imgs, masks_nd, label_dict, imgs_idx, masks_1d = get_imgs_masks_labels(image_dir,
        size=model_size, keyLabel=KEY_LABEL)
    masks_label_names = list(sorted(label_dict.keys()))
    print(f"imgs.shape={imgs.shape}, nd.shape={masks_nd.shape}, 1d.shape={masks_1d.shape}, imgs_idx={imgs_idx.shape}")
    ...
    print(f"ypreds: type={ypreds[0].dtype}")
    ...
    for i in range(masks_nd.shape[0]):
        res = createresult(i, finaloutputdir, imgs_idx, imgs,
                           masks_nd, ypreds, modelPara['label_names'],
