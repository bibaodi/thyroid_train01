...
def best_accuracy_score_and_threshold(y_true, y_score, n_thr=101):
    ...
    Args:
        ...
        y_score: An np.ndarray
            The model score (probability) for each sample
        n_thr: int, optional
...
def validate(dataset, model_callable_name, 
    ...
    with tf.Graph().as_default():
        ...
        mdl = model_func(input_placeholder=img_, input_scale=input_scale, is_training=False, give_summary=True)

        _, pool_out = mdl.outputs
        
        # Loss
        loss = losses.mil_loss_binary(y_, pool_out, class_weights=class_weights)         
        
        # img score
        img_score = scoring.create_score_binary(pool_out) 
