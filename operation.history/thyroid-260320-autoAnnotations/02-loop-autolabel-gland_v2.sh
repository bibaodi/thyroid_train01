...
# Uses realpath if available, otherwise readlink -f, otherwise fallback to cd + pwd.
to_absolute_path() {
    local path="$1"
    if command -v realpath &>/dev/null; then
        ...
        if [ -e "$path" ]; then
            ...
            # If the path does not exist, we cannot resolve it reliably; return as is.
            echo "$path"
        fi
    fi
}
...
# Convert to absolute paths (the script will work with absolute paths from now on)
