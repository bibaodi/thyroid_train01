...
test ! -d ${DEST_DIR} && echo "Source [${DEST_DIR}] not exist" && return 0

# Create the destination directory if it doesn't exist
mkdir -p "$DEST_DIR"

# Move the top 100 items
ls "$SOURCE_DIR" | head -n 1000 | while read file; do
...
move_top_n $1 $2;
## bash moveHeadN2NewFolder.sh ./images/train/ ./images/val/
#
